    ...
    return (
        <div className='flex flex-col items-center mt-32 gap-5'>
            ...
            <div className='flex items-center gap-x-5 w-full max-w-2xl'>
                ...
                {user ? <Link href={`create?title=${logoTitle}`}><Button className='shadow text-base text-white capitalize w-full  p-6 font-semibold hover:text-white hover:bg-rose-600 bg-rose-600'>Get Started</Button></Link>: <SignInButton mode='modal' forceRedirectUrl={`create?title=${logoTitle}`}><Button className='shadow text-base text-white capitalize w-full  p-6 font-semibold hover:text-white hover:bg-rose-600 bg-rose-600'>Get Started</Button></SignInButton>}
            </div>
        </div>
    )
}
